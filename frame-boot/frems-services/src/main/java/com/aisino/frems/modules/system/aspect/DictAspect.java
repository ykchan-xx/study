...
public class DictAspect {
    ...
    // 定义切点Pointcut
    @Pointcut("execution(public * com.aisino.frems.modules..*.*Controller.*(..))")
    ...
    private void parseDictText(Object result) {
        if (result instanceof InvokeResult) {
            if (((InvokeResult) result).getData() instanceof IPage) {
                List<JSONObject> items = new ArrayList<>();
                for (Object record : ((IPage) ((InvokeResult) result).getData()).getRecords()) {
                ...
                ((IPage) ((InvokeResult) result).getData()).setRecords(items);
            }
