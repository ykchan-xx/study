...
import com.aisino.frems.modules.system.model.DuplicateCheckVo;
...
public class DuplicateCheckController {
	...
	@ApiOperation("重复校验接口")
	public InvokeResult doDuplicateCheck(DuplicateCheckVo duplicateCheckVo, HttpServletRequest request) {
		Long num = null;
		...
		if (num == null || num == 0) {
			// 该值可用
			return InvokeResult.success("该值可用！");
		} else {
			// 该值不可用
			log.info("该值不可用，系统中已存在！");
			return InvokeResult.failure("该值不可用，系统中已存在！");
		}
	}
}
