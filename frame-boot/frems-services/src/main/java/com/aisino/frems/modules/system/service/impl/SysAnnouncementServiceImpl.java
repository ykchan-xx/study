...
import com.aisino.frems.modules.system.dao.SysAnnouncementMapper;
import com.aisino.frems.modules.system.dao.SysAnnouncementSendMapper;
...
public class SysAnnouncementServiceImpl extends ServiceImpl<SysAnnouncementMapper, SysAnnouncement> implements ISysAnnouncementService {
	...
	public boolean upDateAnnouncement(SysAnnouncement sysAnnouncement) {
		...
		String userId = sysAnnouncement.getUserIds();
		if(LangUtil.isNotEmpty(userId)&&sysAnnouncement.getMsgType().equals(CommonConstant.MSG_TYPE_UESR)) {
			// 2.补充新的通知用户数据
			...
			for(int i=0;i<userIds.length;i++) {
				...
				if(announcementSends.size()<=0) {
					SysAnnouncementSend announcementSend = new SysAnnouncementSend();
					announcementSend.setId(UUID.randomUUID().toString().replace("-", ""));
					announcementSend.setAnntId(anntId);
					announcementSend.setUserId(userIds[i]);
					announcementSend.setReadFlag(CommonConstant.NO_READ_FLAG);
