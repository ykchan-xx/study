...
import com.aisino.frems.modules.system.dao.SysCategoryMapper;
...
public class SysCategoryServiceImpl extends ServiceImpl<SysCategoryMapper, SysCategory> implements ISysCategoryService {
	...
	public void addSysCategory(SysCategory sysCategory) {
		...
		if(LangUtil.isNotEmpty(sysCategory.getPid())){
	...
	@Override
	public List<TreeSelectModel> queryListByCode(String pcode){
		String pid = ROOT_PID_VALUE;
		if(LangUtil.isNotEmpty(pcode)) {
			List<SysCategory> list = baseMapper.selectList(new LambdaQueryWrapper<SysCategory>().eq(SysCategory::getCode, pcode));
			if(list==null || list.size() ==0) {
				throw new AsRuntimeException("该编码【"+pcode+"】不存在，请核实!");
			}
			if(list.size()>1) {
				throw new AsRuntimeException("该编码【"+pcode+"】存在多个，请核实!");
			}
