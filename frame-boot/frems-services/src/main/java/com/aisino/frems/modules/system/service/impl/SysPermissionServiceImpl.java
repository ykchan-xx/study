...
import com.aisino.frems.modules.system.dao.SysPermissionMapper;
...
public class SysPermissionServiceImpl extends ServiceImpl<SysPermissionMapper, SysPermission> implements ISysPermissionService {
	...
	public void deletePermission(String id) {
		...
		if(sysPermission==null) {
			throw new AsRuntimeException("未找到菜单信息");
		}
		String pid = sysPermission.getParentId();
		if(LangUtil.isNotEmpty(pid)) {
	...
	public void editPermission(SysPermission sysPermission) {
		...
		if(p==null) {
			...
			String pid = sysPermission.getParentId();
			if((LangUtil.isNotEmpty(pid) && !pid.equals(p.getParentId())) || LangUtil.isEmpty(pid)&&LangUtil.isNotEmpty(p.getParentId())) {
				//a.设置新的父菜单不为叶子节点
				this.sysPermissionMapper.setMenuLeaf(pid, 0);
				//b.判断老的菜单下是否还有其他子菜单，没有的话则设置为叶子节点
				int cc = this.count(new QueryWrapper<SysPermission>().lambda().eq(SysPermission::getParentId, p.getParentId()));
				if(cc==0) {
					if(LangUtil.isNotEmpty(p.getParentId())) {
