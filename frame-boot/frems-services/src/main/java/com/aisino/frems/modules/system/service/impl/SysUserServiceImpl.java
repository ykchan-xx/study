...
public class SysUserServiceImpl extends ServiceImpl<SysUserMapper, SysUser> implements ISysUserService {
    ...
    public InvokeResult resetPassword(String username, String oldpassword, String newpassword, String confirmpassword) {
        SysUser user = userMapper.getUserByName(username);
        String passwordEncode = PasswordUtil.encrypt(username, oldpassword, user.getSalt());
        if (!user.getPassword().equals(passwordEncode)) {
            return InvokeResult.failure("旧密码输入错误!");
        }
        if (LangUtil.isEmpty(newpassword)) {
            return InvokeResult.failure("新密码不允许为空!");
        }
        if (!newpassword.equals(confirmpassword)) {
            return InvokeResult.failure("两次输入密码不一致!");
        }
        String password = PasswordUtil.encrypt(username, newpassword, user.getSalt());
		SysUser sysUser = new SysUser();
		sysUser.setPassword(password);
        this.userMapper.update(sysUser, new LambdaQueryWrapper<SysUser>().eq(SysUser::getId, user.getId()));
        return InvokeResult.success("密码重置成功!");
    }

    @Override
    @CacheEvict(value = {CacheConstant.SYS_USERS_CACHE}, allEntries = true)
    public InvokeResult changePassword(SysUser sysUser) {
        ...
        return InvokeResult.success("密码修改成功!");
    }
...
//			// TODO URL规则有问题？
//			if (LangUtil.isNotEmpty(po.getUrl())) {
//				permissionSet.add(po.getUrl());
//			}
			if (LangUtil.isNotEmpty(po.getPerms())) {
...
//		this.save(user);  //保存角色的时候已经添加过一次了
		if(LangUtil.isNotEmpty(selectedParts)) {
	...
	public void editUserWithDepart(SysUser user, String departs) {
		...
		if(LangUtil.isNotEmpty(departs)) {
	 ...
	 */
	@Override
	public InvokeResult checkUserIsEffective(SysUser sysUser) {
		//情况1：根据用户信息查询，该用户不存在
		if (sysUser == null) {
			//sysBaseAPI.addLog("用户登录失败，用户不存在！", CommonConstant.LOG_TYPE_1, null);
			return InvokeResult.failure("500", "该用户不存在，请注册");
		}
		//情况2：根据用户信息查询，该用户已注销
		if (CommonConstant.DEL_FLAG_1.toString().equals(sysUser.getDelFlag())) {
			...
			return InvokeResult.failure("500", "该用户已注销");
		}
		//情况3：根据用户信息查询，该用户已冻结
		if (CommonConstant.USER_FREEZE.equals(sysUser.getStatus())) {
			sysBaseAPI.addLog("用户登录失败，用户名:" + sysUser.getUsername() + ",已冻结！", CommonConstant.LOG_TYPE_1, null);
			return InvokeResult.failure("500", "该用户已冻结");
		}
		return InvokeResult.success();
	}

    @Override
    public List<Map> getUserByRole(Map cond) {
	    if (cond == null || cond.get("roles") == null) {
            return null;
        }
	    return userMapper.getUserByRole(cond);
    }
}
